...
function generateImplementation(
  ...
  const hasStaticHash = Boolean(meta?.hash);
  const lines: string[] = [];
  lines.push(`// Auto-generated by n3-typegen. Do not edit.`);
  if (meta?.hash) {
    lines.push(`// Contract: ${name} (${meta.hash})`);
...
export async function generate(options: GenerateOptions): Promise<void> {
  const { manifest = "", out = "src/contracts" } = options;
  if (!manifest) {
    throw new Error("--manifest path is required");
  }
  const mf = readJsonFile<N3ContractManifest>(manifest);
  ...
  if (options.impl) {
    const implContent = generateImplementation(mf, {
      hash: options.hash,
      nameOverride: options.name,
    });
